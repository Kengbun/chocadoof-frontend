...
const FormArticle = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('http://localhost:5000/article', payload, {
                headers: { 'Content-Type': 'multipart/form-data' },
            });

            // รับ URL ของไฟล์จาก server
            const { coverImageUrl, additionalImageUrl } = response.data;

            // แสดง URL ของภาพที่อัพโหลดจาก server
            setSelectedCoverImage(coverImageUrl);
            setSelectedAdditionalImage(additionalImageUrl);
